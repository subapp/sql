...
FROM
  ...
  LEFT JOIN tableName AS t0 ON (
    (
      t0.id <> t1.subId
      OR t1.id NOT IN(5, 10, 20)
    )
  )
  LEFT JOIN tableName AS t0 USING (t0.id, t1.subId)
  LEFT JOIN (
...
ORDER BY
  activitytypes.orderby ASC,
  activities.starttime ASC
